...
def meineVerarbeitungsFunktion(eingabefeld, system, system2):
    global result, sehen
    system = int(system)
    system2 = int(system2)
...
def SystemSystem():
    ...
    button.clicked.connect(
        lambda x: label.setText(
            str(
                meineVerarbeitungsFunktion(
                    eingabefeld.text(), system.text(), system2.text()
                )
            )
        )
    )
...
def TaschenrechnerSystem():
    ...
    button.clicked.connect(
        lambda x: label.setText(
            str(meineTaschenrechnerfunktion(eingabefeld.text(), system.text()))
        )
    )
    fenster.show()
